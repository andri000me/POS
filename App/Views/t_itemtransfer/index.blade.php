                ...
                <div class="card-header">
                    <div class = "row">
                        ...
                        <div class = "col-6 text-right">
                            <a class = "" href="{{ baseUrl('titemtransfer/add')}}"><i class = "fa fa-plus"></i></a>
                        </div>
                    </div>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                        <table id = "tableitemtransfer" style="width: 100%;" class="table table-striped table-no-bordered table-hover dataTable dtr-inline collapsed " role="grid">
  ...
  <script>
    ...
    function dataTable(){
      table = $('#tableitemtransfer').DataTable({
        ...
        "serverSide": true,
        ajax:{
          url : "{{ baseUrl('titemtransfer/getAllData')}}",
          dataSrc : 'data'
        },
        stateSave: true
      }); 
       ...
       table.on( 'click', '.delete', function (e) {
          ...
          deleteData(name, function(result){
            if (result==true)
            {
              $.ajax({
                type : "POST",
                url : "{{ baseUrl('titemtransfer/delete/')}}",
                data : {id : data['0']},
      ...
      table.on( 'click', '.role', function () {
          ...
          var data = table.row($tr).data();;
          var id = data['Id'];
          window.location = "{{ baseUrl('titemtransfer/editrole/')}}" + id;
      });
      ...
      table.on( 'click', '.reportrole', function () {
          ...
          var data = table.row($tr).data();;
          var id = data['Id'];
          window.location = "{{ baseUrl('titemtransfer/editreportrole/')}}" + id;
      });
    }
